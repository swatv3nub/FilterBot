...
@app.on_message(filters.command(["add"]) & filters.group, group=1)
async def connect(bot: app, update):
    """
    ...
    try:
        ...
        await update.reply_text(f"My User [{userbot_name}](tg://user?id={userbot_id}) Couldnt Join The Channel `{target}` Make Sure Userbot Is Not Banned There Or Add It Manually And Try Again....!!")
        return
...
@app.on_message(filters.command(["del"]) & filters.group, group=1)
async def disconnect(bot: app, update):
    """
    ...
    in_db = db.in_db(chat_id, channel_id, channel_name)
    ...
    db.del_active(chat_id, channel_id, channel_name)
    await ReCacher(chat_id, True, True)
    
    await wait_msg.edit_text("Sucessfully Deleted All Files From DB....")


@app.on_message(filters.command(["delall"]) & filters.group, group=1)
...
@app.on_message(filters.channel & (filters.video | filters.audio | filters.document), group=0)
